 ...
 * - BGM 즉시 정지 → 구간 끝나면 VanishEndBGM을 처음부터 새로 재생
 *   (VanishEndBGM이 비어 있으면 멈췄던 BGM을 이어서 재개)
...
public:
	...
	// 재등장 시 새로 재생할 BGM. 지정하면 끊긴 지점이 아니라 이 사운드를 처음부터 재생.
	// 비워두면 기존처럼 멈췄던 BGM을 이어서 재개.
	UPROPERTY(EditAnywhere, Category = "Vanish")
	TObjectPtr<USoundBase> VanishEndBGM = nullptr;
